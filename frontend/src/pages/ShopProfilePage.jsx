...
export default function ShopProfilePage() {
    ...
    const handleProfileSubmit = async (e) => {
        ...
        try {
            ...
            setProfileMsg('Shop profile details updated successfully!');
            fetchShopProfile();
        } catch (err) {
    ...
    const handleDeleteService = async (serviceId) => {
        if (window.confirm('Are you sure you want to remove this service offering?')) {
            await api.delete(`/shop/services/${serviceId}`);
            fetchShopProfile();
        }
    };
        ...
        <div className="animate-fade max-w-5xl mx-auto my-4">
            ...
            {profileMsg && (
                <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-sm mb-6 flex items-center">
                    <i className="fas fa-check-circle mr-2"></i> {profileMsg}
                </div>
            )}
                                    ...
                                    <tr key={service.id} className="hover:bg-slate-50/50">
                                        ...
                                        <td className="py-3 px-4">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                                                service.is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200'
                                            }`}>
